 ...
 */
export type ITool = "select" | "laser" | "pen" | "text" | "element-eraser" | "duplicate" | "clear" | "undo" | "redo" | "pan" | "image" | "zoomIn" | "zoomOut" | "reset" | "fitToContentDoc" | "zoomLevel" | "firstPage" | "lastPage" | "prevPage" | "nextPage" | "pageInfo" | "preview" | "docUpload" | "docSelect" | "pageBoardInfo" | "fitToContent" | "fitToDoc" | "shapeSelect";

/**
 * 各类图标的私有配置
 */
export type IToolCustomProperty = ICustomStateUnitOption | ICustomPopUnitOption | IPreviewUnitOption | IMultiInOneOption | IDocUploadOption | IShapeSelectOption;
...
export interface IAppConfig {
  ...
  /**
   * 资源使用URL H5页面时，调用 getStream() 推流需要加载URL页面内容,
   * 目前sdk通过MutationObserver监控iframe页面是否有变化，有变化后才会主动抓取ifream的页面内容。
   * 为了防止用户频繁操作ifaeam页面中的内容导致抓图太多频繁（抓图会增加性能消耗），目前增加了一个防抖逻辑。
   *
   * 该变量设置 URL页面内容抓图 的防抖时间间隔。默认为 2000(ms)
   */
  urlRefreshTimerInterval: number;
  /**
