 ...
 * @param
 */
  enableCameraTransform(enable: boolean): void;
  /**
   * 返回当前各类元素的默认样式
   */
  getDefStyles(): IStyle;
   ...
   */
  getResourceMetadata(opt: { url: string; type: "image" | "video" | "audio" }): IResourceMetaData;
  ...
  /**
   * 设置涂鸦（直线）、图形的鼠标样式。
   * @param type 类型'shape' | 'pen'
   * @param cursorImgUrl 鼠标样式图片地址（图片尺寸建议20*20，单位px）
   */
  setStrokeCursor(type: "shape" | "pen", cursorImgUrl: string): void;
  /**
   ...
   * ```
   */
  rotateElement(opt: { id: string; angle: number }): void;
   ...
   * ```
   */
  setElementsVisibility(opt: { ids?: string[]; visible: boolean }): void;
  ...
  exportAsImage(opt?: {
     ...
     */
    content?: "clip" | "fitToDoc" | "fitToContent" | "region";
    /**
     * 是否绘制选择框。默认为false，即不忽略（绘制）
     */
    omitSelection?: boolean;
