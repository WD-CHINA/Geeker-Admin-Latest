...
export function findParents(menu: MenuOptions[], targetItem: MenuOptions): MenuOptions[] {
  const recursiveFind = (menu: MenuOptions[], targetItem: MenuOptions, currentPath: MenuOptions[]): MenuOptions[] | null => {
 ...
 */
export function treeFilter<T, K extends string = "children">(node: TreeLike<T, K>, fn: (_node: TreeLike<T, K>) => boolean, childKey: K = "children" as K): TreeLike<T, K> | null {
  // 递归处理子节点
  const children = node[childKey] ?? [];
  const filteredChildren = children.map(child => treeFilter(child, fn, childKey)).filter((child): child is TreeLike<T, K> => child !== null);
 ...
 * @returns 过滤后的新树
 */
export function treeMap<T, R, K extends string = "children">(tree: TreeLike<T, K>, mapFn: (_node: T) => R, childrenKey: K = "children" as K): TreeLike<R, K> {
...
export async function encryptPassword(password: string) {
  ...
  if (!window.crypto || !window.crypto.subtle) {
    console.warn("crypto.subtle 不可用。请在 HTTPS 或 localhost 环境下运行，或使用降级方案。");
    // 降级方案：简单的 Base64 编码（仅用于开发环境）
    // 生产环境请务必使用 HTTPS
