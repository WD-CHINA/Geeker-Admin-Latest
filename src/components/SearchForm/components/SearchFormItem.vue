<template>
  <component :is="column.search?.render ?? `el-${column.search?.el}`" v-bind="{ ...handleSearchProps, ...attrs, ...placeholder, searchParam: _searchParam, clearable }" v-model.trim="_searchParam[column.search?.key ?? handleProp(column.prop!)]" :data="column.search?.el === 'tree-select' ? columnEnum : []" :options="['cascader', 'select-v2'].includes(column.search?.el!) ? columnEnum : []">
...
const placeholder = computed(() => {
  ...
  const placeholder = search?.attrs?.placeholder ?? (search?.el?.includes("input") ? "请输入" : "请选择") + " " + props.column.label;
  return { placeholder };
});
