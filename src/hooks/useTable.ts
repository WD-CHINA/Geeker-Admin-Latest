...
// 重载1
export function useTable<TableItem>(_api: (_params: IObject) => Promise<ResultPage<TableItem>> | Promise<TableItem[]>, _initParam: object, _pagination: ProTablePaginationEnum, _t: ComposerTranslation, _fePaginationFilterMethod?: (_query: IObject) => IObject[], _dataCallBack?: (_data: TableItem[]) => IObject[]): any;

export function useTable<TableItem>(api: (_params: IObject) => Promise<ResultPage<TableItem>> | Promise<TableItem[]>, initParam: object = {}, pagination: ProTablePaginationEnum = ProTablePaginationEnum.BE, t: ComposerTranslation, fePaginationFilterMethod?: (_query: IObject) => IObject[], dataCallBack?: (_data: TableItem[]) => IObject[]) {
  ...
  const getTableList = async () => {
    try {
      // 先把初始化参数和分页参数放到总参数里面
      Object.assign(state.totalParam, initParam, pagination !== ProTablePaginationEnum.NONE ? { pageNum: state.pageable.pageNum, pageSize: state.pageable.pageSize } : {});
