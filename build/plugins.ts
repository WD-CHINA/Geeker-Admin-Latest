...
import { compressDirToZip } from "./zip";
 ...
 * @param viteEnv
 */
export const createVitePlugins = (viteEnv: ViteEnv, command: ConfigEnv["command"], { buildName, version, lastBuildTime, branch, commitId }): (PluginOption | PluginOption[])[] => {
      ...
      generateFile([
        {
          type: "json",
          output: "./version.json",
          data: { buildName, version, lastBuildTime, branch, commitId }
        }
      ]),
    // 打包压缩
    compressDirToZip(buildName)
  ];
};
...
const createVitePwa = (viteEnv: ViteEnv): PluginOption | PluginOption[] => {
  ...
  return VitePWA({
    ...
    workbox: {
      // 增加缓存文件大小限制到 5 MB（默认是 2 MB）
      // whiteboard/RecordPlayer.js 文件为 2.23 MB
